...
# Prints unicode characters for stones
...
# Allows computer to pick a random pile from the available choices
def random_pile(piles)
  # Piles can't have 0 stones
  eligible_piles = piles.reject { |letter, amount| amount == 0 }

  # Randomly select one of the remaining piles
  selected_pile = eligible_piles.keys.sample.to_sym
end

# Allows computer to pick a random amount of stones from the available choices
def random_stones(piles, selected_pile)
  amount = piles[selected_pile]
  rand(1..amount)
end
...
# Get player names
2.times do |i|
  print "Player #{i + 1} name (or type computer to play an automated opponent): "
...
while play_again
  ...
  while active_game
    ...
    if current_player == "Computer"
      selected_pile = random_pile(piles)
      puts "Computer chooses pile #{selected_pile}."

      stones = random_stones(piles, selected_pile)
      print "Computer will remove #{stones} "
